 ...
 *2022 Beaver Graphical Effects  *2023 Beaver Graphical Effects CE with new features like bevel outlines, thin/bold text mode and shines and so much more.
 * 2022 BarefootLiam (for helping give Inner Glow a disable checkbox) 
 */

/*

To help future devs I will list predicted scenarios that I believe will lead to Graphical effects or any of my plugins breaking compatibility with future versions of Gimp. 
--
1st example of how Graphical effects can and eventually break is if Gimp team adds new blend modes that will over ride "operation", "gimp:layer-mode", "layer-mode", #,
...
--
The second most common reason Graphical Effects will break is if a GEGL operation changes its properties. In example gaussian blur's values are
...
The third most common reason Graphical Effects will break is if a operation is removed or renamed. It will then have to be rebuilt and shipped with Graphical Effects
...
Crude recreation of parts of Graphical Effects graph. This just shows how the nodes are ordered. They are many things missing and I can't possibly list all the options. 
...
/*This is the enum list for parts of Graphical Effects to be displayed (part of filter to be displayed)*/
...
/*Below is sliders and other options that are displayed in Graphical Effects GUI. They are effected by the order they are in and the ui_meta association*/
...
I choose not to use this in Graphical Effects */
...
for opacity control of the second stroke. This is meant to remove a bug where Graphical Effects stacks to copies of the layer on start up.
...
#define GEGL_OP_META
#define GEGL_OP_NAME     graphicaleffectspending
#define GEGL_OP_C_SOURCE graphicaleffectspending.c
...
/* Everything below is the literal GEGL Graph instructions. There are technically four GEGL graphs that load depending on the checkboxes.*/
  ...
  multiply = state->multiply; /* Blend mode switchers for color fill. May 31st 2023 Graphical Effects got the ability to change blend modes for color fill. */
...
graphical effects use to use the default drop shadow until Beaver realized that the only way to give Graphical Effects styles seen in Adobe's layer effects was to abandon gegl:dropshadow and
...
/* Blend modes for Inner Glow are here*/
  ...
  /*over is the switch for for lb:innerglow's blend mode switcher*/
...
gegl_op_class_init (GeglOpClass *klass)
{
 ...
 /*btw, if graphical effects (or any of my plugins) ever breaks try changing the name space from gegl: or lb: to something else.*/
  gegl_operation_class_set_keys (operation_class,
    ...
    "title",       _("Graphical Effects Continual Edition"),
    "reference-hash", "continual45ed565h8500fca01b2ac",
    "description", _("Graphical Effects text styling and specialty image outlining filter. August 1st 2023 Stable Build"
                     ""),
    "gimp:menu-path", "<Image>/Filters/Text Styling",
    "gimp:menu-label", _("Text Styling Effects CE..."),
    NULL);
}
